...
function makeTNSFile(cbOK)
{
    if (!window.lunaLib) {
        alert('WebLuna not ready?!');
        return;
    }
    ajaxAction("getAllSrcFilesContent", `except=${proj.currFile}`, (files) => {
        const otherFileNames = [];
        const otherScriptsContent = [];
        for (const [name, content] of Object.entries(files))
        {
            otherFileNames.push(name);
            otherScriptsContent.push(content);
        }
        const tnsFile = callLunaWithMultipleSrc([ editor.getValue(), ...otherScriptsContent ], [ proj.currFile, ...otherFileNames ], proj.prgmName);
        cbOK(tnsFile);
    });
}

function downloadTnsFile()
{
    makeTNSFile((file) => {
        const blob = new Blob([file], {type: 'application/octet-stream'});
        window['saveAs'](blob, `${proj.prgmName}.tns`);
    });
}
