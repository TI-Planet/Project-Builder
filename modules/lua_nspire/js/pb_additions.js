...
function renameFile(oldName)
{
    $('.tooltip').hide();
    let err = false;
    const newName = prompt("Enter the new file name (Chars: A-Z,0-9 Extension: lua)", oldName);
    if (newName === null || !isValidFileName(newName))
    {
        err = true;
        if (newName) {
            showNotification("danger", "Invalid name", "Chars: A-Z,0-9 Extension: lua");
        }
    }
    if (newName === oldName) {
        return;
...
{
    return name === 'icon.png' || /^[A-Z0-9]+\.lua$/i.test(name);
}
...
function makeTNSFile()
{
    if (!window.lunaLib) {
        alert('WebLuna not ready?!');
        return;
    }
    return callLuna(editor.getValue(), proj.prgmName);
}

function downloadTnsFile()
{
    const file = makeTNSFile();
