...
<?php } else { ?>
    ...
    <script type="module">
        ...
        window.callLuna = function(inputStr, fileName)
        {
            const luaName = `${fileName}.lua`;
            const tnsName = `${fileName}.tns`;
            lunaLib.FS.writeFile(luaName, inputStr);
            try { lunaLib.FS.unlink(tnsName); } catch (e){}
            if (lunaLib.callMain([luaName, tnsName]) !== 0) {
...
<script src="<?= cacheBusterPath('js/codemirror/lua.js') ?>"></script>
