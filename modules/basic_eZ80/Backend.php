...
final class basic_eZ80ProjectBackend extends PHPBasedBackend
{
    ...
    public function __construct(basic_eZ80Project $project, $projFolder)
    {
        ...
        $this->projPrgmExtension = '8xp';
    }

    public function getAvailableSrcFiles()
    {
        $availableFiles = array_filter(array_map('basename', glob($this->projFolder . 'src/*.*')), '\ProjectBuilder\basic_eZ80Project::isFileNameOK');
        sort($availableFiles);
    ...
    private function saveSource($source, $baseSourceHash = null)
    {
        ...
        $validation = $this->validateExpectedFileHash($baseSourceHash, $filePath, self::TEMPLATE_BASIC_FILE_PATH);
        if ($validation !== true)
        {
            return $validation;
        }
    ...
    public function getCurrentFileSourceHTML()
    {
        ...
        $templateFile = self::TEMPLATE_BASIC_FILE_PATH;
    ...
    public function getCurrentFileSourceHash()
    {
        $currFile = $this->project->getCurrentFile();
        $sourceFile = $this->projFolder . 'src/' . $currFile;
        return $this->getTextFileHashWithFallback($sourceFile, self::TEMPLATE_BASIC_FILE_PATH) ?? '';
    }
